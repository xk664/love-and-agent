...
> 设计原则：克制、精确、有温度。不做信息堆砌，每个元素只做一件事。
...
**基础搭建**：
- [ ] 项目初始化，配置 Vite 构建
- [ ] 安装依赖：Element Plus、Axios、Vue Router、Pinia
- [ ] Axios 封装：统一请求/响应拦截、Token 注入、401 处理
- [ ] 路由配置：四个主页面（首页、恋爱大师、超级智能体、知识库）
- [ ] Pinia 状态管理：用户状态、会话状态
- [ ] 全局样式体系：CSS 变量、响应式断点
...
**设计意图**：入口要干净，不要让注册成为负担。

- [x] 登录页：用户名 + 密码，表单验证，错误状态反馈
- [x] 注册页：用户名 + 密码 + 确认密码，注册成功自动跳转登录
- [x] Token 管理：localStorage 存储，Axios 拦截器自动注入
- [x] 状态持久化：刷新不丢失登录态
- [x] 401 兜底：Token 过期自动跳转登录页
...
**设计意图**：导航是产品的骨架，不要让它抢戏。

- [x] 主布局：侧边栏 + 顶部栏 + 内容区，结构清晰
- [x] 响应式三档：
  - 桌面（≥1024px）：完整侧边栏
  - 平板（768-1023px）：可收起侧边栏
  - 移动端（<768px）：底部 Tab 或抽屉导航
- [x] 导航项：首页、恋爱大师、超级智能体、知识库
- [x] 当前路由高亮，过渡动画
...
**设计意图**：会话列表是用户的历史记忆，要让人一眼找到想继续的对话。

- [x] 会话列表：标题 + 最后活跃时间 + 应用类型标签
- [x] 创建会话：选择类型（恋爱大师/超级智能体），恋爱大师需选情感状态
- [x] 会话操作：删除（二次确认）、切换、新建
- [x] 分页加载，空状态引导
...
**设计意图**：情感状态是恋爱大师的上下文入口，选择要轻量但有意义。
...
**设计意图**：对话是产品核心体验，消息流要自然，流式响应要有呼吸感。

- [ ] 消息气泡：用户/AI 样式区分，时间戳，AI 头像
- [x] 输入区：文本框 + 发送按钮，Enter 发送，Shift+Enter 换行
- [ ] SSE 流式接收：
  - `thinking` 事件：显示思考状态
  - `answer` 事件：逐句渲染，打字机效果
  - `metadata` 事件：展示 token 用量、RAG 来源
  - `[DONE]` 标记：流结束，恢复输入
- [ ] 历史消息：分页加载，自动滚动到底部
- [ ] 空会话引导语
...
**设计意图**：智能体任务是异步的，状态变化要让人感知到进展，而不是干等。

- [ ] 任务输入：描述输入框 + 提交按钮
- [ ] 任务状态面板：
  - pending：等待中
  - running：执行中，显示当前步骤
  - completed：展示最终结果和执行步骤
  - failed：错误信息
  - cancelled：已取消
- [x] 轮询机制：每 3 秒查询状态，完成后停止
- [x] 取消操作：pending/running 状态可取消
...
**设计意图**：知识库是后台管理类页面，效率优先，不要过度设计。

- [ ] 文档列表：标题、文件类型、状态（待处理/已向量化/失败）
- [ ] 文档上传：支持 .md / .pdf / .txt，上传进度，成功后刷新列表
- [ ] 文档删除：二次确认
- [ ] 向量化重试：仅失败文档显示重试按钮
- [ ] 分页加载，空状态提示
